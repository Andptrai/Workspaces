...
function addDataToHTML() {
    ...
    let freezeProductIds = [8,9,7]; // Thay thế bằng danh sách ID cà phê thực tế của bạn

    // Lọc chỉ hiển thị sản phẩm có ID trong danh sách coffeeProductIds
    let freezeProducts = products.filter(product => freezeProductIds.includes(product.id));

    freezeProducts.forEach(product => {
