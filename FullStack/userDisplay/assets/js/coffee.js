...
function addDataToHTML() {
    ...
    // Danh sách các ID cà phê bạn muốn hiển thị
    let coffeeProductIds = [1, 2, 3,6]; // Thay thế bằng danh sách ID cà phê thực tế của bạn

    // Lọc chỉ hiển thị sản phẩm có ID trong danh sách coffeeProductIds
    let coffeeProducts = products.filter(product => coffeeProductIds.includes(product.id));
    ...
    coffeeProducts.forEach(product => {
        ...
        newProduct.innerHTML += `
            ...
            <button class="addToCartButton" data-product-id="${product.id}">Thêm</button>`;
            

        listProductHTML.appendChild(newProduct);

        // Add event listener to open cart when clicking the "Add to Cart" button
